...
async function sendVerificationEmail(account: any, origin: any) {
    ...
    if (origin) {
        ...
        message = `<p>Please use the below token to verify your email address with the /account/verify-email API route:</p>
                   <p><code>${account.verificationToken}</code></p>`;
    }

    await sendEmail({
        to: account.email,
        subject: 'Signup Verification API - Verify Email',
...
async function sendAlreadyRegisteredEmail(email: any, origin: any) {
    ...
    await sendEmail({
        to: email,
        subject: 'Signup Verification API - Email Already Registered',
...
async function sendPasswordResetEmail(account: any, origin: any) {
    ...
    if (origin) {
        ...
        message = `<p>Please use the below token to reset your password with the <code>/account/reset-password</code> API route, the token will be valid for 24 hours:</p>
                   <p><code>${account.resetToken}</code></p>`;
    }

    await sendEmail({
        to: account.email,
        subject: 'Signup Verification API - Reset Password',
